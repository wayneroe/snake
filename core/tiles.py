...
class Tiles(object):
    ...
    def calculate_offsets(self):
        tiles_size = self.map_size * self.tile_size
    ...
    def calculate_rects(self, x_offset, y_offset):
        tile_rects = np.empty((self.map_size, self.map_size), pg.Rect)

        for i in range(self.map_size):
            for j in range(self.map_size):
                tile_rect = [x_offset + i * self.tile_size, y_offset + j * self.tile_size,
                             self.tile_size, self.tile_size]
                tile_rects[i][j] = tile_rect
